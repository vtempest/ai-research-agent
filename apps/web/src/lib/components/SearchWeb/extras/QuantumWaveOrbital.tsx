...
export default function QuantumWaveOrbital({
  ...
  const randomRange = useCallback((min: number, max: number): number => 
    min + random() * (max - min), [random]);
  
  const randomInt = useCallback((min: number, max: number): number => 
    Math.floor(randomRange(min, max + 1)), [randomRange]);
   ...
   */
  const generateSphereConfig = useCallback((cfg: OrbitalSphereConfig = config): SphereData => {
  ...
  const handleSphereClick = useCallback((event: React.MouseEvent<HTMLDivElement>): void => {
    if (onSphereClick) {
      onSphereClick();
    }
  }, [onSphereClick]);
  ...
  const getLineStyle = useCallback((line: OrbitalLine): LineStyle => {
  ...
  }, [autoRandomize, config, handleMouseMove, randomizeSphere, shiftHue, randomRange]);

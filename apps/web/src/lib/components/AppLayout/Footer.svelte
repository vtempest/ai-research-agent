...
<div
  class="absolute bottom-2 left-1/2 -translate-x-1/2 text-slate-200 text-sm z-20 {optionBackgroundColor} rounded-lg px-2 py-1 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-wrap items-center justify-center gap-x-4 max-w-[90vw]"
>
  {#each listFooterLinks as { url, text, icon }}
    ...
    >
      ...
      {/if}
      <span class="font-semibold tracking-wide uppercase text-xs">{text}</span>
      <span

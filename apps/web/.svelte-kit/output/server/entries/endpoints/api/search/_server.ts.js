...
import "@langchain/community/chat_models/togetherai";
import "@langchain/xai";
import "@langchain/google-vertexai-web";
import "../../../../chunks/BlockType.js";
import "js-yaml";
import { getDomainWithoutSuffix } from "tldts";
import { c as convertURLSafeHTMLToHTML } from "../../../../chunks/html-utils.js";
import "linkedom";
import "jszip";
import "katex";
import "marked";
...
async function searchWeb(query, options = {}) {
  ...
  if (privateSearxng) {
    ...
    results = results.map((result) => {
      ...
      return { title, url: url2, snippet, score };
    });
    return results.map((result) => {
      var favicon = "https://www.google.com/s2/favicons?domain=" + result.url.match(/^(?:https?:\/\/)?(?:www\.)?([^/:?\s]+)(?:[/:?]|$)/i)?.[0] + "&sz=16";
      ...
      var path = result.url?.replace(/(http:\/\/|https:\/\/|www.)/gi, "").split("/").slice(1).join("/").replace(/^/, "/");
      return {
        ...result,
        domain,
        source,
  ...
  results = results.map((result) => {
    var favicon = "https://www.google.com/s2/favicons?domain=" + result.url.match(/^(?:https?:\/\/)?(?:www\.)?([^/:?\s]+)(?:[/:?]|$)/i)?.[0];
    var domain = result.url?.replace(/(http:\/\/|https:\/\/|www.)/gi, "").split("/")[0];
    var path = result.url?.replace(/(http:\/\/|https:\/\/|www.)/gi, "").split("/").slice(1).join("/").replace(/^/, "/");
    return {
      ...result,

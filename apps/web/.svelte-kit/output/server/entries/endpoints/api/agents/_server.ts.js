...
import { L as LANGUAGE_MODELS, a as LANGUAGE_PROVIDERS, A as AGENT_TOOLS, c as convertURLSafeHTMLToHTML, b as convertMarkdownToHTML } from "../../../../chunks/BlockType.js";
import "js-yaml";
import "tldts";
import "linkedom";
import "jszip";
...
async function generateLanguageResponse(options = {}) {
  ...
  try {
    ...
    const LangModelProvider = provider === "groq" ? new ChatGroq({ apiKey, model, temperature }) : provider === "togetherai" ? new TogetherAI({ apiKey, model, temperature }) : provider === "openai" ? new ChatOpenAI({ openAIApiKey: apiKey, model, temperature }) : provider === "anthropic" ? new ChatAnthropic({

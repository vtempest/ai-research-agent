...
Defined in: apps/web-app/src/lib/components/AppLayout/auth-google-one-tap.js:51
...
\{ `auto_select?`: `boolean`; `use_fedcm_for_prompt?`: `boolean`; `cancel_on_tap_outside?`: `boolean`; `callback_url`: `string`; `state_cookie_domain?`: `string`; \}
...
Defined in: apps/web-app/src/lib/components/AppLayout/auth-google-one-tap.js:107

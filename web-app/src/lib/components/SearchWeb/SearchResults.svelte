<script lang="ts">
  ...
  let { searchResultList, selectedResultIndex,
     currentArticle, fetchArticle, 
     loadMoreResults, isLoading }:
  { searchResultList: Array<SearchResult>,
     selectedResultIndex: number, currentArticle:
      Article | null, fetchArticle: 
      (url: string, index: number) => void, 
      loadMoreResults: () => void, isLoading: 
...
function handleResultClick(index) {
  ...
  fetchArticle(searchResultList[index].url, index);
}
</script>
...
{#if searchResultList.length > 0}
  <div class="space-y-1">
    <!-- // @ts-ignore -->
    {#each searchResultList as { title, url, favicon, domain, snippet} , index}

import { AttributeMap, Line } from '../document';
...
export class Typeset {
  ...
  constructor(types: TypesetTypes) {
    ...
    const formats = types.formats
      ?.map(entry => (typeof entry === 'string' ? formatTypes[entry] : entry))
      .filter(Boolean);
...
export class Types<T extends BasicType = BasicType> {
  ...
  init() {
    this.selector = this.list
      ...
      .filter(Boolean)
      .join(', ');
    this.types = this.list.reduce((types, type) => {
      types[type.name] = type;
      return types;
    }, {} as TypeMap<T>);
    this.priorities = this.list.reduce(
      (priorities, type, i) => {
        priorities[type.name] = i;
        return priorities;
      },

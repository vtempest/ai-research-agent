/**
 * @fileoverview Utility for converting OpenAPI specifications into AI agent tools.
 * Generates LangChain-compatible tools from YAML specs.
 */
...
export function convertOpenAPIToLangChainTools(fileContents) {
  ...
  function convertToZodSchema(schema, isRequired = false) {
    ...
    if (
      !isRequired &&
      !zod.includes(".optional()") &&
      !zod.includes(".default(")
    ) {
      zod += ".optional()";
    }
  ...
  function generateTool(name, desc, schema, pathInfo, op) {
    ...
    const body = {};
    ${bodyProps.map((p) => `if (params.${p} !== undefined) body.${p} = params.${p};`).join("\n    ")}
    
    const fullUrl = baseUrl + url + (queryParams.toString() ? '?' + queryParams.toString() : '');
    
    try {
      return await grab(fullUrl, {
        method: '${method.toUpperCase()}',
        headers,
        ${bodyProps.length ? `body: Object.keys(body).length > 0 ? JSON.stringify(body) : undefined,` : ""}
      });
    } catch (error) {

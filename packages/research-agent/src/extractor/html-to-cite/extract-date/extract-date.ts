/**
 * @fileoverview Comprehensive date extraction orchestrator for HTML documents.
 * Integrates pattern searching, JSON-LD parsing, and image metadata analysis.
 */
import {
  discard_unwanted,
  extract_url_date,
...
/**
 * Format a date object into the desired output format
 */
function format_date(date: Date, format: string): string {
  return convert_date(date, "YYYY-MM-DD", format);
}
...
function search_page(htmlstring, options) {
  ...
  if (bestmatch) {
    const dateObject = new Date(
      parseInt(bestmatch[1]),
      parseInt(bestmatch[2]) - 1,
      1
    );
    if (
  ...
  if (
    dateObject &&
    is_valid_date(dateObject, options.format, options.min, options.max) &&
    (copyear === 0 || dateObject.getFullYear() >= copyear)
  ) {
    try {
...
class Counter extends Map {
  ...
  *elements() {
  ...
  update(iterable) {
    const other =
      iterable instanceof Counter ? iterable : new Counter(iterable);
...
export function extractDate(
  ...
  if (extensive_search) {
    ...
    const textNodes = Array.from(search_tree.childNodes).filter(
      (node: any) => node.nodeType === 3
    );

    // Now iterate through them
    for (let i = 0; i < textNodes.length; i++) {
      const segment = (textNodes[i] as any).textContent.trim();
      if (
        segment.length > MIN_SEGMENT_LEN &&

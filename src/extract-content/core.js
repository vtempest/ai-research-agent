    ...
    if (options.outputFormat.includes('xml')) {
        ...
        if (options.withMetadata) {
            ...
            ['title', 'author', 'url', 'hostname', 'description', 'sitename',
             'date', 'categories', 'tags', 'fingerprint', 'id', 'license'].forEach(attr => {
                if (document[attr]) {
...
function trafilaturaSequence(cleanedTree, cleanedTreeBackup, treeBackup, options) {
...
function bareExtraction(filecontent, options) {
    // try {
        ...
        if (options.withMetadata) {
            document = extractMetadata(tree, options.url, options.dateParams, options.fast, options.authorBlacklist);
            ...
            if (options.onlyWithMetadata && !(document.date && document.title && document.url)) {
        ...
        let [postbody, tempText, lenText] = trafilaturaSequence(cleanedTree, cleanedTreeBackup, tree, options);
        ...
        if (options.outputFormat === 'python') {
            ...
            if (options.comments) {
                document.comments = document.commentsbody ? document.commentsbody.textContent : '';
            }
...
function extract(htmlOrDom, options = {}) {
        ...
        var document = typeof htmlOrDom == "string" ?
            (new JSDOM(htmlOrDom)).window.document : htmlOrDom;
    ...
    // try {
        ...
        if (!TXT_FORMATS.has(extractorOptions.outputFormat)) {
            if (extractorOptions.outputFormat === "python") {
                throw new Error("'python' format only usable in bareExtraction() function");
            }

 ...
 * @category Generate
 * @returns {{prompt: string; name: string; tools: any[], variablesNotProvided: [], error: string}} An object with agent prompts.
...
4. Represent All Nodes: Make sure all nodes are included in the edge list
  ...
  var prompt = agentPrompts.filter((prompt) => prompt.name == agentName)?.[0];

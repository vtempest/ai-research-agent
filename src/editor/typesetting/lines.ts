import { AttributeMap, Delta, normalizeRange } from '../document';
...
export const list = line({
  ...
  fromDom(node: HTMLElement) {
    let indent = -1,
      ...
      type = parent && (parent as Element).getAttribute('type');
  ...
  renderMultiple: (lists, editor, forHTML) => {
    ...
    function compareLists(list: VNode, type: string, attributes: AttributeMap) {
      return (
        list.type === type &&
        (list.props.start === attributes.start || (list.props.start && !attributes.start)) &&
...
export const hr = line({
  ...
  commands: editor => () => {
    ...
    if (range[0] === range[1] && doc.getLineAt(range[0]).length === 1) {
      change.insert(range[0], '\n', { ...doc.getLineFormat(range[0]) }).formatLine(range[0], { hr: true });
    } else {

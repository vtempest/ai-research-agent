...
const patchDom = (parent: Node, dom: Node, oldDom: Node | null, newVdom: VChild, isSvg?: boolean) => {
  if (typeof newVdom === 'string') {
    ...
    for (var i in { ...oldProps, ...newProps }) {
      if ((domProps.has(i) ? dom[i as keyof Node] : oldProps[i]) !== newProps[i]) {
...
const patchChildren = (
  ...
  newVKids: VChild[],
  isSvg?: boolean,
  oldKids: Node[] = Array.from(dom.childNodes) as Node[]
) => {
  var tmpKid: Node,
    oldKid: Node,
    oldKey: any,
  ...
  while (newHead <= newTail && oldHead <= oldTail) {
    if ((oldKey = getKey(oldKids[oldHead])) == null || oldKey !== getKey(newVKids[newHead])) {
      break;
    }
  ...
  while (newHead <= newTail && oldHead <= oldTail) {
    if ((oldKey = getKey(oldKids[oldTail])) == null || oldKey !== getKey(newVKids[newTail])) {
      break;
    }
  ...
  if (oldHead > oldTail) {
    ...
    while (newHead <= newTail) {
      ...
      if (newKeyed.has(oldKey) || (newKey != null && newKey === getKey(oldKids[oldHead + 1]))) {
        if (oldKey == null) {
...
const vdomify = (vdom: any): VChild => (vdom !== true && vdom !== false && vdom ? vdom : '');
...
export const h: H = (type: string | Function, props?: Props | null, ch?: VChild | VChild[]) =>
  typeof type === 'function'
    ? type(props || {}, ch)
    : createVdom(type, props || {}, Array.isArray(ch) ? ch : ch == null ? [] : [ch], props?.key);

import { AttributeMap, Delta, Line, TextChange, TextDocument, isEqual, type EditorRange } from '../document';
...
export class Decorator {
  ...
  constructor(
    name: string,
    doc: TextDocument,
    decoration: Delta | undefined,
    apply: (name: string, updates: Delta) => void,
  ...
  clearLines(lines: Line[]) {
    ...
    const contiguous =
      lines.length === 1 ||
      lines.every((line, i) => !i || doc.getLineRange(lines[i - 1])[1] === doc.getLineRange(line)[0]);
    if (contiguous) {
...
export function applyDecorations(vnode: VNode, attributes: AttributeMap | undefined, defaultClasses?: string[]) {
  ...
  Object.values(attributes.decoration as Record<string, Decorations>).forEach((decorations: Decorations) => {

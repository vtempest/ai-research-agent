 ...
 */
  ...
  /**
 * Creates a comprehensive YouTube player emulator with full API support.
 * @returns {YouTubePlayer} An object  containing the YouTube-like API functionality.
 * @category Extractor
 * @example 
  const YT = createYouTubeEmulator();
...
*/
function createYouTubeEmulator() {
    ...
    function triggerEvent(eventName, eventData) {
      if (eventListeners[eventName]) {
        eventListeners[eventName].forEach(listener => listener({ data: eventData }));
      }
    }
      ...
      Player: function(elementId, options) {
        ...
        return {
          ...
          loadPlaylist: (playlistOrVideoIds, index, startSeconds) => {
            playlist = Array.isArray(playlistOrVideoIds) ? playlistOrVideoIds : [playlistOrVideoIds];
